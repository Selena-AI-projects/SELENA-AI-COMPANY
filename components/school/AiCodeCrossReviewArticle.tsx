...
const roles = [
  {
    name: "Владелец",
    description:
      "Определяет задачу, цели, ограничения и то, что нельзя нарушать. Принимает решения о деньгах, доступах, публикации, merge и production.",
  },
  {
    name: "ChatGPT",
    description:
      "Помогает собрать контекст, документы и требования в понятное техническое задание. ChatGPT не заменяет репозиторий и не определяет, какой commit проверяется.",
  },
  {
    name: "Codex",
    description:
      "Работает с репозиторием: реализует задачу в отдельной ветке, добавляет тесты и готовит точный diff к независимой проверке.",
  },
  {
    name: "GitHub",
    description:
      "Хранит код, commits, pull requests и результаты автоматических проверок. Это общий источник истины для всех участников процесса.",
  },
  {
    name: "Claude Code",
    description:
      "Получает то же ТЗ и точную версию кода. Проводит независимый read-only аудит, не изменяя исходный репозиторий и не опираясь заранее на выводы Codex.",
  },
  {
    name: "Автоматические тесты",
    description:
      "Проверяют только заложенные сценарии: типы, сборку, тесты и другие машинные gates проекта. Зелёные проверки не гарантируют отсутствие всех ошибок.",
  },
] as const;
...
const ownerGates = [
  ...
  "миграция общей staging- или production-базы",
  "merge в release или main",
  "production deploy",
  "публичная публикация",
  "запуск платежей, billing и recurring jobs",
    ...
    <article>
      ...
      >
        <p className="max-w-3xl border-l-2 border-copper pl-5 text-[1.05rem] leading-8 text-ink/80">
          Этот workflow нужен владельцам бизнеса и начинающим AI-разработчикам, которые не могут профессионально проверить каждую строку кода. Codex реализует задачу, Claude Code независимо проверяет тот же commit, GitHub фиксирует версию, а человек сохраняет контроль над необратимыми решениями.
        </p>
      ...
      <section className="bg-surface pb-16 pt-10 sm:pb-24">
        <Container size="narrow">
          <div className="space-y-5 text-[1.04rem] leading-8 text-ink/78">
            ...
            <p>
              Отдельно хранила контекст проекта, копировала задания из одного чата в другой и сама пыталась следить, какую версию кода кто проверял. После этого разговора я решила первым делом убрать именно эту ручную часть и собрать единый workflow.
            </p>
          </div>
      ...
      <section className="bg-ivory py-16 sm:py-24" aria-labelledby="who-needs-cross-review">
        <Container size="narrow">
          ...
          <div className="mt-6 space-y-5 text-[1.04rem] leading-8 text-ink/78">
            ...
            <p>
              Усиленный контроль особенно нужен перед изменениями в базе данных, миграциях, авторизации, правах доступа, персональных данных, платежах, безопасности, публикации, staging и production.
            </p>
          </div>
      ...
      <section className="bg-ivory py-16 sm:py-24" aria-labelledby="one-task-workflow">
        <Container size="narrow">
          ...
          <h2 id="one-task-workflow" className="mt-4 font-serif text-3xl font-semibold text-ink sm:text-4xl">
            Как контролировать AI-разработку от ТЗ до owner-gate
          </h2>
          <p className="mt-6 text-[1.04rem] leading-8 text-ink/78">
            Последовательность важна: независимая проверка кода Claude Code относится к конкретному commit, а не к «последней версии где-то в чате».
          </p>
          ...
          <ResponsiveArticleFigure
            ...
            alt="Цикл задачи: постановка ТЗ, реализация Codex, GitHub CI, независимая проверка Claude Code и owner-gate."
            title="Цикл проверки AI-кода: Codex, GitHub, Claude Code и owner-gate"
            caption="Цикл задачи с возвратом замечаний к Codex, остановкой BLOCKED_PLAN_LIMIT и решением владельца перед merge или публикацией."
            width={1080}
            height={1350}
          />
          ...
          <CrossReviewWorkflow />
          <p className="mt-7 border-l-2 border-copper pl-5 font-medium leading-7 text-ink">
            Короткий вывод: у каждой проверки есть точная версия, а любой новый commit делает предыдущий review устаревшим.
          </p>
        </Container>
      </section>
      ...
      <section className="bg-surface py-16 sm:py-24" aria-labelledby="github-source-of-truth">
        <Container size="narrow">
          ...
          <p className="mt-7 border-l-2 border-copper pl-5 font-medium leading-7 text-ink">
            Короткий вывод: совпадение мнений не является доказательством; доказательство начинается с одного и того же commit, diff и результатов проверок.
          </p>
        </Container>
      </section>
      ...
      <section className="bg-surface py-16 sm:py-24" aria-labelledby="workflow-limits">
        <Container size="narrow">
          ...
          <div className="mt-6 space-y-5 text-[1.04rem] leading-8 text-ink/78">
            <p>
              Этот workflow не означает работу без остановки 24/7. Компьютер может уснуть, интернет — пропасть, CI — остановиться, а лимит Claude Max — временно закончиться. Поэтому корректная формулировка звучит так: автономный workflow с сохранением состояния, повторным продолжением и остановкой на owner-gates.
            </p>
            <p>
              Нельзя обещать «полностью автономную разработку без человека», гарантированное отсутствие ошибок, подтверждение работоспособности всего кода одними тестами или автоматическое исправление кода Claude Code. Независимый review тоже может ошибиться.
            </p>
          </div>
          <p className="mt-7 border-l-2 border-copper pl-5 font-medium leading-7 text-ink">
            Короткий вывод: цель workflow — сделать риск и ответственность видимыми, а не объявить AI безошибочным.
          </p>
        </Container>
      </section>
        ...
        <Container size="narrow">
          ...
          <ul className="mt-8 space-y-4">
            {[
              ...
              ["Как читать AI Visibility Report и проверять evidence", "/ru/lab/guides/read-ai-visibility-report-evidence"],
              ["Бесплатная Public Readiness-проверка сайта", "/ru/check"],
      ...
      <section className="bg-charcoal py-16 text-ivory sm:py-20" aria-labelledby="article-discussion">
        <Container size="narrow">
          ...
          <p className="mt-5 max-w-3xl text-lg leading-8 text-ivory/72">
            Я продолжаю проверять этот workflow на собственных проектах и дорабатывать границы между автоматической работой и решениями владельца. Расскажите, что вы проверяете сами, что передаёте второй системе и где оставляете обязательное решение за человеком.
          </p>
          <p className="mt-5 max-w-3xl text-lg leading-8 text-ivory/72">
