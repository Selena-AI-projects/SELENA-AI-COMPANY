...
export function MeasurementFilmstrip({ content }: { content: MeasurementContent }) {
  ...
  const onScroll = useCallback(() => {
    ...
    // Wide slides mean the track runs out of scroll before the last slide
    // reaches `(n-1) * step`, so the end of the track IS the last slide.
    if (track.scrollLeft >= track.scrollWidth - track.clientWidth - 4) {
      setIndex(content.slides.length - 1);
      return;
    }
        ...
        <Reveal delay={100}>
          ...
          >
            {content.slides.map((slide, slideIndex) => (
              <figure
                key={slide.title}
                className="w-[86%] shrink-0 snap-start overflow-hidden rounded-3xl border border-ivory/12 bg-charcoal-2 sm:w-[64%] lg:w-[47%]"
              >
                <div className="relative sm:aspect-[16/10]">
                  ...
                  {/* Below sm the caption is a normal block under the frame —
                      an overlay would be taller than the 16:10 media box on a
                      narrow phone and clip. From sm up it overlays the frame. */}
                  <figcaption className="p-5 sm:absolute sm:inset-x-0 sm:bottom-0 sm:bg-gradient-to-t sm:from-charcoal sm:via-charcoal/55 sm:to-transparent sm:p-7 sm:pt-20">
