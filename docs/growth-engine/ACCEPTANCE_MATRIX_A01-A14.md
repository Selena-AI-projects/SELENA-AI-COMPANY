...
| A09 | Backoff/restart: хотя бы один реальный интервал backoff и перезапуск worker без ручной правки next-attempt; событие не теряется | **live** | Шесть отложений с удвоением 30 с → 16 мин (лог `68054aa6`), замена контейнера воркера посреди ожидания (`88310594`), проекция в назначенный срок 03:59:29 под новой политикой — подробности в разделе «Жизненный цикл политики» |
...
| A12 | Миграции: фактический upgrade-путь, hashes/objects, no-op, конкурентный запуск | **live** + sample | Staging: журнал 38 → 41 вручную по SHA, хеши записей = файлам, 16 проверок объектов инспектора `yes`, повтор — no-op (репетиция на копии фактического состояния). Раннер отказывает при тихом пропуске (GD-08). Инспектор показывает чужие сессии и DDL-блокировки перед запуском; автодеплой `migrate` снят на время работ. Резервные копии `963c2c4b`, `4c8891a1`, `bafef901`; восстановление логического архива проверено (GD-09). `0041` применена тем же регламентом 04:12 UTC (инспектор до: единственная ожидающая, без чужих сессий; после: журнал 42, ожидающих нет) |
...
6. **Проекция без потери**: свежий контейнер `88310594` в 03:59:29 (срок был
   ...
   попытки не правился.
7. **Финальная проверка** (`verify` @ `a9f79560`, 04:14:51, после `0041` и
   пересборки `web`): событий 20 — десять `content.draft_ready` спроецированы
   (восемь с `attempts 0`, два с `attempts 6`), десять `task.result.ready` не
   тронуты; материалов бренда 10 = 5 brief × 2, версий > 1 — 0; brief
   `5ec3b3f0` (тот, что ждал) — обе версии под **`staging-check-2`**; brief
   `88b9d4d3` — под `staging-check-1` (след дефекта GD-12, до исправления);
   `approvals`, `release_intents`, `release_manifests`, `publication_attempts`,
   `workflow_dispatches` = 0; материалов под другими брендами 0; Inbox
   владельца — 10 строк, Review — 0.

**A09 закрыт live.** Дополнительно закрыты по review (GD-13): прямые записи
web-рантайма в `content_policies` сняты миграцией `0041` (на staging применена
04:12:34, журнал 42, инспектор до и после — без чужих сессий и блокировок);
черновики Control Room получают действующую политику сервером.
