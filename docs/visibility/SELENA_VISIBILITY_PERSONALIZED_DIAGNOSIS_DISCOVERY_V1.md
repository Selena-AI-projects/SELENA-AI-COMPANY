...
#### 8.2а. Live-сценарий обеих групп после activation (2026-09-23) — попытка выполнена, заблокирована на сети

После activation (D-034) нужно было отдельно подтвердить живое поведение на реальном публичном эндпоинте: control не должен доходить до LLM, experimental должен либо показать объяснение, либо корректно деградировать без ошибки. Использован не новый диагностический роут, а сам существующий публичный `POST /api/checks` — с подобранным cookie `selena_exp`, чтобы детерминированно попасть в нужную группу.

Формула из `lib/visibility/security/bucketing.ts` (`assignVariant`): `SHA256(cookieValue:experimentId)`, первый байт по модулю 2 → чётный = `control`, нечётный = `personalized_explanation`, где `experimentId = "visibility_personalized_explanation_v1"`. Подобраны значения:

```text
control                    → 1c842a8f-28fa-45a8-b045-7d9e4466ecc7
personalized_explanation   → 6cade6e9-2e84-4b50-89c3-2e92944f019f
```

```bash
curl -sS -X POST https://www.selenasystems.com/api/checks \
  -H "Content-Type: application/json" \
  -H "Cookie: selena_exp=1c842a8f-28fa-45a8-b045-7d9e4466ecc7" \
  -d '{"website":"https://www.selenasystems.com","primaryAction":"other","siteProfile":"all_checks","locale":"en"}'

curl -sS -X POST https://www.selenasystems.com/api/checks \
  -H "Content-Type: application/json" \
  -H "Cookie: selena_exp=6cade6e9-2e84-4b50-89c3-2e92944f019f" \
  -d '{"website":"https://www.selenasystems.com","primaryAction":"other","siteProfile":"all_checks","locale":"en"}'
```

Ожидаемый результат: `variant` в ответе совпадает с намеченной группой (само по себе подтверждает, что формула бакетирования верна); control → `explanation: null`; experimental → `ok: true`, полный `report`, а `explanation` либо непустой массив (успех), либо `null` (документированная деградация) — оба варианта корректны, 500 или неполный `report` — нет.

Это намеренно **не** проходит через браузер: прямой `POST /api/checks` не импортирует ни `track`/`trackPublicEvent`, ни `@vercel/analytics` нигде по цепочке вызовов (`resolveExplanation.ts` → `generate.ts` → `liveReport.ts` — все проверены, ни одного упоминания аналитики), и ничего не пишет в БД. Событие `personalized_explanation_viewed` стреляет только из `LiveReportView.tsx` при реальном рендере в браузере. Поэтому такой вызов **структурно** не попадает ни в конверсию, ни в аналитику — не потому, что помечен флагом, а потому что этот путь их физически не затрагивает.

**Попытка выполнена 2026-09-23, заблокирована на сети** — первый (control) запрос вернул `403 Forbidden` от egress-прокси этой среды (`curl: (56) CONNECT tunnel failed, response 403`), та же самая блокировка `selenasystems.com`, что и весь этот рабочий проход. Второй запрос не выполнялся — результат идентичен по построению. Обе команды выше готовы к копированию — нужно, чтобы их выполнил владелец (или любая среда, у которой есть доступ к домену) и прислал JSON-ответ.
...
Активация эксперимента (`SELENA_VISIBILITY_DECISION_LOG.md`, D-034) состоялась **до** закрытия этого пункта — владелец сделала это осознанно. Пункт остаётся открытым и не отменяется этим решением — просто больше не блокирует activation.

**[Исправлено 2026-09-23]** Первая версия этого абзаца называла ≈$0.07/день «худшим случаем» для всего сайта — это неверно, и вот почему. `dailyExplanationCount`/`dailyExplanationDay` (дневной soft cap) и `explanationBuckets` (часовой лимит на IP) в `lib/visibility/security/rate-limit.ts` — обычные module-level переменные (`let`, `Map`), без KV/БД/внешнего хранилища (прочитано полностью, строки 100, 126–155). На serverless это означает: у каждого «тёплого» инстанса — свой независимый счётчик. $0.00035/вызов (реальная средняя цена из §8.2) × 200/день — это ≈$0.07/день **на один инстанс**, а не на весь сайт. Реальный худший случай для сайта в целом — `N × $0.07/день`, где N (сколько инстансов Vercel держит одновременно тёплыми) этим кодом не наблюдается и не ограничивается. In-memory защита реально тормозит одного вышедшего из-под контроля клиента; она не даёт проверяемого потолка суммарных трат. Единственный проверяемый потолок — hard spend limit на стороне OpenAI, и именно поэтому §8.5 остаётся значимым открытым пунктом, а не формальностью.

OpenAI с 2026-07-22 действительно поддерживает настоящий hard limit на уровне проекта/организации (после которого запросы получают `429 project_spend_limit_exceeded`), но это исключительно дашборд-настройка: **Project settings → Limits → Spend → Edit spend limit → указать месячную сумму → включить тумблер «Enforce a hard limit»**. Программного API для установки или чтения этого лимита нет (проверено — Usage/Costs API покрывают только чтение трат, не лимиты). Это значит: ни поставить, ни подтвердить этот лимит отсюда невозможно ни при каких правах доступа — это исключительно действие владельца в дашборде OpenAI. Статус пункта остаётся `OPEN`, не переобозначается как «уведомление отправлено» или что-либо близкое к «закрыто».

### 8.6. VERCEL_WEB_ANALYTICS_NOT_ENABLED — измерительный слой инертен

**Статус: включение (переключатель в дашборде) — RESOLVED 2026-09-22. Реальная доставка событий (хотя бы одно событие фактически получено и видно в дашборде с ожидаемыми свойствами) — NOT CONFIRMED, по состоянию на 2026-09-23.**

**[Исправлено 2026-09-23]** Эти два факта — независимые, и путать их нельзя: «включён переключатель» не означает «события реально доходят и видны». Ранее в чате (после активации D-034) это было один раз суммировано как «§8.6 ✅ RESOLVED» без оговорки — это было неточно, доставка событий этим не подтверждена и не проверялась. Исправляю здесь явно: статус доставки остаётся NOT CONFIRMED, а не «resolved» и не мягкое «needs_verification», которое читается как «почти готово».
...
**Не подтверждено и по состоянию на 2026-09-23** — из среды разработки: реально ли долетают события до дашборда, и с какими свойствами. Проверено заново в этом проходе (2026-09-23): прямые запросы к `https://api.vercel.com/v1/user`, `https://vercel.com/` и `https://www.selenasystems.com/` — все три вернули `403 Forbidden` от egress-прокси этой среды, не от сервера. Это значит, что даже при наличии Vercel API-токена запрос к `GET /v1/query/web-analytics/events/aggregate` (реальный REST-эндпоинт Vercel Web Analytics для агрегатов по custom events, с разбивкой `by=eventName`) выполнить отсюда всё равно нельзя — блокировка на уровне сети, не credentials.

Единственный работающий путь подтвердить получение аналитики: (а) владелец открывает Vercel → Analytics → Events, ищет `personalized_explanation_viewed` и `readiness_complete`, смотрит реальные свойства события; либо (б) владелец (или любая среда, способная достучаться до `vercel.com`) выполняет тот же REST-запрос с Vercel API-токеном и присылает результат. Пока этого не произошло — статус остаётся NOT CONFIRMED, а не «resolved» с оговоркой.
