...
**Статус: OPEN, вне репозитория. Блокирует §8.1 и приёмку эксперимента целиком.**

Обнаружено 2026-09-21 при разборе первого успешного деплоя: в дашборде проекта `selena-ai-company` карточка **Web Analytics** имеет состояние **Not Enabled**.

D-018 был закрыт выбором Vercel Web Analytics как провайдера, и код под это написан: `components/layout/SiteShell.tsx` монтирует `<Analytics />` из `@vercel/analytics/next`, а `lib/diagnostics/analytics.ts` вызывает `track()` из `@vercel/analytics`. Но включение провайдера в коде — только половина; вторая половина это переключатель в настройках проекта. Пока он выключен, Vercel не отдаёт `/_vercel/insights/script.js`, объект `window.va` в браузере не появляется, и `track()` остаётся тем же no-op, каким был до D-018.

Следствие, которое нужно сказать явно, а не подразумевать: **`readiness_complete`, `personalized_explanation_viewed` и остальные события сегодня не записываются**. Эксперимент `visibility_personalized_explanation_v1` в таком состоянии неизмерим — обе ветки дадут ноль, и отличить «объяснение не помогло» от «данных нет» будет нельзя. Включать `VISIBILITY_PERSONALIZED_EXPLANATION_ENABLED` до этого момента — значит платить провайдеру за трафик, результат которого невозможно прочитать.

Порядок снятия блокеров поэтому фиксированный: `8.6 → 8.1 → 8.4 → 8.5 → 8.2 → activation`. §8.1 (лимит количества custom-event properties) физически нельзя проверить в дашборде, пока раздел аналитики не активирован.

Проверка того, что переключатель действительно подействовал, из среды разработки недоступна: `vercel.com` и production-домен закрыты egress-прокси (попытки `curl` возвращают 403 от прокси, не от сайта), токена Vercel в среде нет. Подтверждение — наблюдение владельца в дашборде либо появление `/_vercel/insights/script.js` в сетевой панели браузера на production-странице.
