...
const RU: SampleReportContentV2 = {
  ...
  recommendationEvidence: {
    title: "3. Доказательства рекомендаций",
  ...
  actionReadiness: {
    ...
    states: [
      ...
      },
      {
        ...
        state: "fail",
        sourceStatus: "sample",
        finding: "Нет документированного API, нет структурированного API-метода для действия, подтверждение — только письмо для человека.",
        doesNotProve:
    ...
    agentCaveat:
      "Видимая кнопка действия, номер телефона или ссылка WhatsApp демонстрируют только готовность для человека. «Выполнимо агентом» — отдельное, более строгое утверждение, и измеряется отдельно.",
  },
  topBlocker: {
    ...
    item: {
      ...
      evidence:
        "Демонстрационное доказательство: поток бронирования не предоставляет структурированного API-метода или параметров, а единственное подтверждение — письмо для человека.",
      whyItMatters:
  ...
  verificationLoop: {
    ...
    stages: [
      ...
      { id: "assigned_action", label: "Назначенная задача" },
      { id: "recheck", label: "Повторная проверка" },
      { id: "verified_outcome", label: "Проверенный результат" },
      { id: "weekly_telegram_report", label: "Еженедельный отчёт в Telegram" },
    ],
    window: {
      baseline: "Исходный замер: цикл C1, 10 августа 2026 (пример)",
      recheck: "Повторная проверка: цикл C2, 17 августа 2026 (пример)",
      lock: "Фиксация конфигурации v1 · 25 вопросов · 3 системы · 4 повтора · сопоставимо",
    },
    columns: {
      action: "Задача",
      owner: "Владелец",
      status: "Статус",
      evidenceIds: "Идентификаторы доказательств",
    ...
    rows: [
      {
        ...
        status: "VERIFIED",
        evidenceIds: ["EV-C1-0412", "EV-C1-0418", "EV-C2-0507"],
        recheck: "Повторный обход тех же 3 страниц, затем те же 25 вопросов в тех же 3 системах с фиксацией конфигурации v1 в C2",
      ...
      },
      {
        ...
        status: "NEEDS_RECHECK",
        evidenceIds: ["EV-C1-0431"],
        recheck: "Проверка пути действия на странице бронирования в C3; обход C2 прошёл до выкладки изменения",
      ...
      {
        id: "ACT-C1-03",
        action: "Опубликовать страницу вопросов и ответов с прямыми ответами и ссылкой с главной",
        owner: "Владелец бизнеса",
        status: "STILL_OPEN",
        evidenceIds: ["EV-C1-0407", "EV-C1-0409"],
        recheck: "Те же 25 вопросов с фиксацией конфигурации v1 в C3; цитирования собственных страниц считаются по каждой системе",
        before: "0 из 25 ответов цитируют собственную страницу (C1)",
        after: "Не начато — перенесено с 34-й недели, а не выдано заново как новая находка",
        sourceStatus: "sample",
      },
      {
        id: "ACT-C1-04",
        ...
        after: "Правило убрано; 4 из 4 страниц получены (C2). Подтверждено в C2, закрыто на 35-й неделе",
        sourceStatus: "sample",
      },
      {
        id: "ACT-C2-05",
        ...
        status: "NEW",
        evidenceIds: ["EV-C2-0519", "EV-C2-0523"],
        recheck: "Те же 2 вопроса с фиксацией конфигурации v1 в C3, цитирования конкурента считаются по каждой системе",
    ...
    delivery: {
      heading: "Доставка дайджеста · 35-я неделя",
      recipient: "Один подтверждённый получатель, приватный чат, привязан одноразовой ссылкой (пример)",
      dueAt: "Срок: понедельник 09:00 по времени проекта; крайний срок — через 6 ч после назначенного времени",
      status: "RETRY_SCHEDULED",
      attempts: [
    ...
    disclosure:
      "«Подтверждено» означает, что тот же зафиксированный набор вопросов, систем и повторов наблюдал изменение в более позднем цикле. Это не доказывает, что изменение вызвала именно задача, и не предсказывает следующий цикл.",
  },
  boundaries: {
    ...
    measured: [
      ...
      "Готово ли основное действие для человека, машиночитаемо ли оно и выполнимо ли агентом",
    ],
    notMeasured: [
