...
const RU: Record<RuleId, Recommendation> = {
  ...
  "identity.jsonld_valid": {
    ...
    doesNotProve:
      "Валидная разметка Schema.org — машиночитаемое подтверждение, а не фактор ранжирования или цитирования.",
  },
  "identity.jsonld_entity_present": {
    severity: "important",
  ...
  "action.agent_executable": {
    ...
    action:
      "Если агентский трафик для вас важен — откройте документированный API-метод с разметкой schema.org potentialAction / EntryPoint и возвращайте машинопроверяемое подтверждение.",
    doesNotProve:
  ...
  "citability.block": {
    ...
    action:
      "Начните с одного прямого предложения-ответа, затем в том же блоке добавьте конкретные границы предложения, локацию, условия или доказательство под содержательным заголовком.",
    doesNotProve:
      "Более цитируемый блок улучшает готовность, но не доказывает, что AI-система его процитирует.",
  },
  "content.readiness": {
    severity: "important",
    title: "Видимый контент слишком краткий или слабо структурирован",
    action:
      "Добавьте короткое описание бизнеса с прямым ответом в первой фразе: предложение, предложения, аудитории, локации и следующего действия и разделите его содержательными заголовками.",
  ...
  "LA-03": {
    ...
    action:
      "Укажите реальные часы работы видимым текстом и, если есть структурированные данные, в openingHours или openingHoursSpecification.",
  ...
  "LA-05": {
    severity: "later",
    title: "Структурированные данные типа LocalBusiness отсутствуют или неполны",
    action:
      "Объявите в JSON-LD сущность типа LocalBusiness с подтверждёнными полями name, address и telephone или openingHours, используя только подтверждённые факты.",
    doesNotProve:
      "Полная разметка LocalBusiness не доказывает появление бренда в Ask Maps или локальных AI-ответах.",
  },
  "LA-06": {
    severity: "later",
    title: "Факты из структурированных данных не подтверждаются видимым контентом",
    action:
      "Повторяйте каждый заявленный в структурированных данных факт — имя, телефон, локацию — в видимом тексте страницы, чтобы разметка и контент не противоречили друг другу.",
    doesNotProve:
  ...
  "LA-07": {
    ...
    action:
      "Если подтверждённая карточка бизнеса в Google Maps существует, дайте на неё ссылку с сайта как на публичный источник. Никогда не ссылайтесь на карточку, которой не управляете.",
    doesNotProve:
...
const PASSED_RU: Record<RuleId, string> = {
  ...
  "LA-05": "Структурированные данные LocalBusiness содержат минимальный набор фактов",
  "LA-06": "Структурированные данные согласованы с видимым контентом",
...
const READINESS_EVIDENCE: Record<VisibilityLocale, Record<ReadinessEvidenceKey, string>> = {
  ...
  ru: {
    ...
    no_agent_signal:
      "Не найдено ни документированного API-метода, ни точки входа действия, ни машинопроверяемого подтверждения",
  },
};
