/**
 * The 8 productized service modules (developer TZ §10).
 * Copy is Russian, concrete, and honest — no fake metrics or guarantees.
 */

export type Service = {
  id: string;
  /** Short editorial number shown on the card, e.g. "01". */
  index: string;
  name: string;
  /** One-line outcome/promise. */
  cardTitle: string;
  promise: string;
  /** 2–4 concrete deliverables the client receives. */
  included: string[];
  /** Who this fits. */
  audience: string;
  /** Where the CTA leads (a dedicated page if one exists, else the brief). */
  href: string;
  ctaLabel: string;
  visual: {
    src: string;
    alt: string;
    label: string;
  };
};

export const services: Service[] = [
  {
    id: "audit",
    index: "01",
    name: "AI-аудит",
    cardTitle: "Понять, с чего начать",
    promise:
      "Разбираем процессы и находим задачи, где AI реально экономит время, а не добавляет хаоса.",
    included: [
      "Карта повторяющихся задач и узких мест",
      "3–5 приоритетных AI-сценариев с оценкой усилий",
      "Понятный план: с чего начать первым",
    ],
    audience: "Тем, кто чувствует пользу AI, но не знает, с чего начать.",
    href: "/contact",
    ctaLabel: "Разобрать процесс",
    visual: {
      src: "/media/services/audit.png",
      alt: "Кинематографичный AI-аудит: карта процессов, сигналы из CRM, частых вопросов, контента и команды сходятся в диагностическую карту.",
      label: "Диагностический кадр",
    },
  },
  {
    id: "training",
    index: "02",
    name: "AI-обучение",
    cardTitle: "Единые правила работы",
    promise:
      "Переводим AI из экспериментов в практику на ваших реальных задачах.",
    included: [
      "Практические сессии на ваших задачах",
      "Библиотека проверенных промптов под процессы",
      "Инструкции для одинакового результата",
    ],
    audience: "Когда каждый пробует по-своему и результат нестабилен.",
    href: "/ai-training",
    ctaLabel: "Настроить практику",
    visual: {
      src: "/media/services/training.png",
      alt: "Кинематографичный кадр AI-обучения: роли, промпты и правила проверки собираются в общую систему работы команды.",
      label: "Командная практика",
    },
  },
  {
    id: "automation",
    index: "03",
    name: "AI-автоматизация",
    cardTitle: "Заявки идут по сценарию",
    promise:
      "Собираем связки без сложного кода для обращений, коммуникаций и данных.",
    included: [
      "Связки на Make / Zapier и Telegram / WhatsApp",
      "Автоответы и черновики на частые обращения",
      "Передача данных между CRM, Notion и таблицами",
    ],
    audience: "Бизнесу, где заявки и сообщения обрабатываются вручную.",
    href: "/ai-automation",
    ctaLabel: "Собрать связку",
    visual: {
      src: "/media/services/automation.png",
      alt: "Кинематографичный кадр AI-автоматизации: заявка проходит через правила, черновик ответа и CRM с передачей человеку.",
      label: "Сценарий процесса",
    },
  },
  {
    id: "content",
    index: "04",
    name: "AI-контент-система",
    cardTitle: "Один материал становится системой",
    promise:
      "Раскладываем экспертную идею в набор форматов без выгорания и хаотичных рывков.",
    included: [
      "Контент-система: один материал → несколько форматов",
      "Шаблоны и промпты под ваш стиль и темы",
      "Процесс с редактурой и проверкой человеком",
    ],
    audience: "Экспертам и брендам, которые ведут контент рывками.",
    href: "/ai-content",
    ctaLabel: "Собрать систему",
    visual: {
      src: "/media/services/content.png",
      alt: "Кинематографичный кадр AI-контент-системы: одна экспертная идея раскладывается в пост, письмо, гайд и бриф.",
      label: "Редакционная система",
    },
  },
  {
    id: "concierge",
    index: "05",
    name: "AI-консьерж",
    cardTitle: "Ответы появляются без ручного шума",
    promise:
      "Собираем первую линию клиентских ответов с обязательной проверкой перед запуском.",
    included: [
      "Сбор частых вопросов: цены, запись, адрес, формат",
      "Сценарий ответов для Telegram / WhatsApp",
      "Правила, когда подключается человек",
    ],
    audience: "Сервисному бизнесу, где админ отвечает на одно и то же.",
    href: "/contact",
    ctaLabel: "Разобрать линию ответов",
    visual: {
      src: "/media/services/concierge.png",
      alt: "Кинематографичный кадр AI-консьержа: типовые клиентские вопросы переходят в быстрые ответы с границей передачи человеку.",
      label: "Клиентская линия",
    },
  },
  {
    id: "knowledge",
    index: "06",
    name: "AI-база знаний",
    cardTitle: "Один источник вместо чатов",
    promise:
      "Переносим правила, частые вопросы и процессы в структуру, из которой можно отвечать одинаково.",
    included: [
      "Структура базы в Notion или удобном инструменте",
      "Единые промпты и регламенты для команды",
      "Понятный доступ и порядок обновления",
    ],
    audience: "Командам, где знания разбросаны по чатам и головам.",
    href: "/contact",
    ctaLabel: "Навести порядок",
    visual: {
      src: "/media/services/knowledge.png",
      alt: "Кинематографичный кадр AI-базы знаний: единый источник правил, частых вопросов, тона коммуникации и процессов.",
      label: "Единый источник",
    },
  },
  {
    id: "packaging",
    index: "07",
    name: "AI-упаковка",
    cardTitle: "Оффер становится ясным",
    promise:
      "Помогаем упаковать продукт или личный бренд с помощью AI — от структуры смыслов до материалов.",
    included: [
      "Структура продукта и ключевых смыслов",
      "Черновики страниц, описаний и офферов",
      "Материалы, готовые к финальной редактуре",
    ],
    audience: "Экспертам и личным брендам на этапе упаковки.",
    href: "/ai-content",
    ctaLabel: "Упаковать оффер",
    visual: {
      src: "/media/services/packaging.png",
      alt: "Кинематографичный кадр AI-упаковки: оффер, доказательства и страница собираются в ясную продуктовую структуру.",
      label: "Упаковка оффера",
    },
  },
  {
    id: "support",
    index: "08",
    name: "AI-сопровождение",
    cardTitle: "Система не разваливается после запуска",
    promise:
      "Поддерживаем систему после запуска: тестируем, дорабатываем сценарии и обучаем новых людей.",
    included: [
      "Регулярный разбор, что работает и что нет",
      "Доработка сценариев и промптов",
      "Обучение новых сотрудников работе с системой",
    ],
    audience: "Тем, кто уже внедрил AI и хочет, чтобы он не разваливался.",
    href: "/contact",
    ctaLabel: "Поддерживать систему",
    visual: {
      src: "/media/services/support.png",
      alt: "Кинематографичный кадр AI-сопровождения: цикл проверки, доработки и обучения новых людей после запуска.",
      label: "Цикл улучшения",
    },
  },
];

export const getService = (id: string) => services.find((s) => s.id === id);
